...
is_safe_part1 :: proc(s: string) -> bool {
    ...
    prev := lib.unsafe_string_to_int(prev_str)
    curr := lib.unsafe_string_to_int(curr_str)
    asc := curr > prev
    ...
    for vs in strings.split_iterator(&s_mut, " ") {
        prev = curr
        curr = lib.unsafe_string_to_int(vs)
        if !is_pair_safe(prev, curr, asc) {
            return false
        }
    }
...
is_safe_part2 :: proc(s: string) -> bool {
    ...
    for str in strings.split_iterator(&s_mut, " ") {
        sa.push_back(&sa_xs, lib.unsafe_string_to_int(str))
    }
    xs := sa.slice(&sa_xs)
